...
public:
    ...
    inline auto MapPointsInMapRaw() const -> size_t {
        std::unique_lock<std::mutex> lock(const_cast<Map*>(this)->mMutexMap);
        return mspMapPoints.size();
    }
    inline auto KeyFramesInMapRaw() const -> size_t {
        std::unique_lock<std::mutex> lock(const_cast<Map*>(this)->mMutexMap);
        return mspKeyFrames.size();
    }

    template <class Fn>
    void ForEachMapPoint(Fn&& fn) const {
        std::unique_lock<std::mutex> lock(const_cast<Map*>(this)->mMutexMap);
    ...
    template <class Fn>
    void ForEachKeyFrame(Fn&& fn) const {
        std::unique_lock<std::mutex> lock(const_cast<Map*>(this)->mMutexMap);
