...
System::System(const string &strVocFile, const string &strSettingsFile,
    ...
    : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)),
      mptLocalMapping(static_cast<std::thread *>(NULL)),
      mptLoopClosing(static_cast<std::thread *>(NULL)),
      mptViewer(static_cast<std::thread *>(NULL)), mbReset(false),
...
void System::Shutdown() {
  auto trace_state = [this](const char *phase) {
    std::ostringstream oss;
    oss << phase << " mbShutDown=" << (mbShutDown ? 1 : 0)
        << " viewer_ptr=" << (mpViewer ? 1 : 0)
        << " viewer_thread=" << (mptViewer ? 1 : 0)
        << " viewer_joinable=" << ((mptViewer && mptViewer->joinable()) ? 1 : 0)
        << " viewer_finished="
        << ((mpViewer) ? (mpViewer->isFinished() ? 1 : 0) : -1)
        << " lm_thread=" << (mptLocalMapping ? 1 : 0) << " lm_joinable="
        << ((mptLocalMapping && mptLocalMapping->joinable()) ? 1 : 0)
        << " lm_finished="
        << ((mpLocalMapper) ? (mpLocalMapper->isFinished() ? 1 : 0) : -1)
        << " lc_thread=" << (mptLoopClosing ? 1 : 0) << " lc_joinable="
        << ((mptLoopClosing && mptLoopClosing->joinable()) ? 1 : 0)
        << " lc_finished="
        << ((mpLoopCloser) ? (mpLoopCloser->isFinished() ? 1 : 0) : -1);
    shutdown_trace::log("orb.system.shutdown", oss.str());
  };
  ...
  trace_state("enter");
  {
    unique_lock<mutex> lock(mMutexReset);
    if (mbShutDown) {
      shutdown_trace::log("orb.system.shutdown", "already_shutdown");
      return;
    }
    mbShutDown = true;
  }
  trace_state("flag_set");

  cout << "Shutdown" << endl;

  if (mpViewer) {
    shutdown_trace::log("orb.system.shutdown", "request viewer finish");
    mpViewer->RequestFinish();
  }
  mpLocalMapper->RequestFinish();
  mpLoopCloser->RequestFinish();
  trace_state("finish_requested");

  if (mpViewer) {
    while (!mpViewer->isFinished())
      usleep(5000);
    trace_state("viewer_finished");
  }

  while (!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished()) {
    trace_state("waiting_workers");
    usleep(5000);
  }
  trace_state("workers_finished");

  if (mptViewer && mptViewer->joinable()) {
    shutdown_trace::log("orb.system.shutdown", "joining viewer thread");
    mptViewer->join();
    trace_state("viewer_joined");
  }
  if (mptLocalMapping && mptLocalMapping->joinable()) {
    shutdown_trace::log("orb.system.shutdown", "joining local mapping thread");
    mptLocalMapping->join();
    trace_state("lm_joined");
  }
  if (mptLoopClosing && mptLoopClosing->joinable()) {
    shutdown_trace::log("orb.system.shutdown", "joining loop closing thread");
    mptLoopClosing->join();
    trace_state("lc_joined");
  }

  if (!mStrSaveAtlasToFile.empty()) {
    trace_state("before_save_atlas");
    Verbose::PrintMess("Atlas saving to file " + mStrSaveAtlasToFile,
                       Verbose::VERBOSITY_NORMAL);
    SaveAtlas(FileType::BINARY_FILE);
    trace_state("after_save_atlas");
  }
...
auto System::GetAtlas() -> Atlas * { return mpAtlas; }

auto System::GetAtlas() const -> const Atlas * { return mpAtlas; }
...
void System::DebugDumpAtlasStats(std::ostream &os) const {
  os << "[orb.system] system_ptr=" << this << " mpAtlas=" << mpAtlas
     << " load_path=" << mStrLoadAtlasFromFile << "\n";
  if (!mpAtlas) {
    os << "[orb.system] atlas is null\n";
    return;
  }
  mpAtlas->DebugDump(os);
}
