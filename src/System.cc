...
void System::Shutdown() {
    auto trace_state = [this](const char* phase) {
        ...
        oss << phase << " mbShutDown=" << (mbShutDown ? 1 : 0)
            ...
            << " viewer_thread=" << (mptViewer ? 1 : 0) << " viewer_joinable="
            << ((mptViewer && mptViewer->joinable()) ? 1 : 0)
            << " viewer_finished="
    ...
    if (mptLocalMapping && mptLocalMapping->joinable()) {
        shutdown_trace::log("orb.system.shutdown",
                            "joining local mapping thread");
        mptLocalMapping->join();
        trace_state("lm_joined");
    }
    if (mptLoopClosing && mptLoopClosing->joinable()) {
        shutdown_trace::log("orb.system.shutdown",
                            "joining loop closing thread");
        mptLoopClosing->join();
