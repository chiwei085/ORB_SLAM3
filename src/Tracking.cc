...
void Tracking::newParameterLoader(Settings* settings) {
    ...
    static std::atomic<bool> s_logged_settings_trace{false};
    if (!s_logged_settings_trace.exchange(true)) {
        std::ostringstream oss;
        oss << "newParameterLoader"
            << " sensor=" << static_cast<int>(mSensor)
            << " insertKFsWhenLost=" << (mInsertKFsLost ? 1 : 0)
            << " imuFreq=" << mImuFreq
            << " depthMapFactor=" << settings->depthMapFactor();
        shutdown_trace::log("orb.settings", oss.str());
    }
#endif
