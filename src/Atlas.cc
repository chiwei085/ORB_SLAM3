...
void Atlas::DebugDump(std::ostream &os) {
  unique_lock<mutex> lock(mMutexAtlas);
  os << "[orb.atlas] atlas_ptr=" << this << " current_map_ptr=" << mpCurrentMap
     << " mspMaps.size=" << mspMaps.size()
     << " mspBadMaps.size=" << mspBadMaps.size()
     << " mvpBackupMaps.size=" << mvpBackupMaps.size() << "\n";

  auto dump_map = [&](const char *label, Map *pMap) {
    if (!pMap) {
      os << "[orb.atlas] " << label << " map_ptr=null\n";
      return;
    }
    os << "[orb.atlas] " << label << " map_ptr=" << pMap
       << " id=" << pMap->GetId()
       << " raw_mspMapPoints=" << pMap->MapPointsInMap()
       << " raw_mspKeyFrames=" << pMap->KeyFramesInMap()
       << " is_bad=" << pMap->IsBad() << " is_in_use=" << pMap->IsInUse()
       << "\n";
  };

  dump_map("current", mpCurrentMap);

  int idx = 0;
  for (Map *pMap : mspMaps) {
    std::ostringstream label;
    label << "mspMaps[" << idx++ << "]";
    dump_map(label.str().c_str(), pMap);
  }

  idx = 0;
  for (Map *pMap : mvpBackupMaps) {
    std::ostringstream label;
    label << "mvpBackupMaps[" << idx++ << "]";
    dump_map(label.str().c_str(), pMap);
  }
}
